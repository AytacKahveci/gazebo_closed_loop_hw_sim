...
*/
#include <gazebo_closed_loop_hw_sim/gazebo_closed_loop_hw_sim.h>
...
bool ClosedLoopHWSim::initSim(const std::string& robot_namespace,
                              ...
                              std::vector<transmission_interface::TransmissionInfo> transmissions)
{
    ...
    pid_controllers_.resize(n_dof_); // Passive joints should be removed from list
    ...
    for(unsigned int j = 0; j < n_dof_; j++)
    {
        ...
        {
            ROS_WARN_STREAM_NAMED("closed_loop_hw_sim", "Transmission" << transmissions[j].name_
            ...
            !(transmissions[j].actuators_[0].hardware_interfaces_.empty()))
        {
            ...
            ROS_WARN_STREAM_NAMED("closed_loop_hw_sim", "The <hardware_interface> element of tranmission " <<
        ...
        if (joint_interfaces.empty())
        {
            ROS_WARN_STREAM_NAMED("closed_loop_hw_sim", "Joint " << transmissions[j].joints_[0].name_ <<
        ...
        // Debug
        ROS_DEBUG_STREAM_NAMED("closed_loop_hw_sim","Loading joint '" << joint_names_[j]
        ...
        else if(hardware_interface == "PassiveJointInterface" || hardware_interface == "hardware_interface/PassiveJointInterface")
        {
            joint_control_methods_[j] = PASSIVE;
            ROS_INFO_STREAM_NAMED("closed_loop_hw_sim", "Passive joint interface found for " << joint_names_[j]);
        }
        else
        {
            ROS_FATAL_STREAM_NAMED("closed_loop_hw_sim","No matching hardware interface found for '"
        ...
        if (!joint)
        {
            ROS_ERROR_STREAM_NAMED("closed_loop_hw_sim", "This robot has a joint named \"" << joint_names_[j]
        ...
        {
            ROS_WARN_STREAM_NAMED("closed_loop_hw_sim", "No physics type found.");
        }

        if(joint_control_methods_[j] != PASSIVE)
            registerJointLimits(joint_names_[j], joint_handle, joint_control_methods_[j],
                                joint_limit_nh, urdf_model,
                                &joint_types_[j], &joint_lower_limits_[j], &joint_upper_limits_[j],
                                &joint_effort_limits_[j]);

        if (joint_control_methods_[j] != EFFORT && joint_control_methods_[j] != PASSIVE)
        {
    ...
    ROS_INFO("ClosedLoopHWSim is loaded successfully");
    return true;
}


void ClosedLoopHWSim::readSim(ros::Time time, ros::Duration period)
{
    for(unsigned int j=0; j < n_dof_; j++)
    {
...
void ClosedLoopHWSim::writeSim(ros::Time time, ros::Duration period)
{
...
void ClosedLoopHWSim::eStopActive(const bool active)
{
...
void ClosedLoopHWSim::registerJointLimits(const std::string& joint_name,
...
PLUGINLIB_EXPORT_CLASS(gazebo_ros_control::ClosedLoopHWSim, gazebo_ros_control::RobotHWSim)